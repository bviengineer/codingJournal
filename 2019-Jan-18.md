...
```
* The 2nd challenge involved writing a method that accepts a number as a parameter and returns an array of positive numbers that were <= the parameter and the paramter, considering the parameter were divisible by the numbers in the array. 
```ruby
#Logic
  def factors(num)
	arr = []
